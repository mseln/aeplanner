...
{
Rrt::Rrt(const ros::NodeHandle &nh) : nh_(nh),
                                      frame_id_("map"),
...
void Rrt::execute(const rrtplanner::rrtGoalConstPtr &goal)
{
  ...
  {
    Eigen::Vector3d *g = new Eigen::Vector3d(goal->goal_poses.poses[i].position.x, goal->goal_poses.poses[i].position.y, goal->goal_poses.poses[i].position.z);
